...
export function InsightsTab() {
  ...
  const generateVipMessage = (client: ClientData) => {
    const messages = [
      `Olá ${client.name}! 🌟 Você é nosso cliente VIP com ${formatCurrency(client.total)} em compras! Obrigado pela sua fidelidade! 💜`,
      `${client.name}, você é incrível! 👑 Já são ${formatCurrency(client.total)} em pedidos conosco. Que tal experimentar nosso prato especial hoje?`,
      `Parabéns ${client.name}! 🎉 Você está entre nossos clientes mais especiais. Preparamos algo especial para você! 💫`,
      `Oi ${client.name}! 💜 Você já investiu ${formatCurrency(client.total)} em momentos deliciosos conosco. Obrigado por escolher sempre a gente!`
    ];
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                onClick={() => setWhatsappMessage(generateVipMessage(selectedClient!))}
              >
                Gerar Nova Mensagem
              </Button>
            </div>
