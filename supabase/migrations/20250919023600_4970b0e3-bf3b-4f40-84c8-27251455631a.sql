-- Fix remaining RLS issues - enable RLS on tables that still need it

-- Check which tables still need RLS enabled based on the remaining errors
-- These tables likely don't have RLS enabled yet:
ALTER TABLE public.produtos ENABLE ROW LEVEL SECURITY;

-- Add missing policies for produtos (already has a SELECT policy, need INSERT/UPDATE/DELETE)
CREATE POLICY "Allow authenticated users to manage produtos" 
ON public.produtos 
FOR ALL 
TO authenticated 
USING (true) 
