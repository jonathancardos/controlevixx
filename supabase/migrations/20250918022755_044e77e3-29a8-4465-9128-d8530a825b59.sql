...
CREATE POLICY "Allow authenticated read access to categorias_produtos" 
ON public.categorias_produtos FOR SELECT 
TO authenticated
USING (true);

CREATE POLICY "Allow authenticated all access to categorias_produtos" 
...
CREATE POLICY "Allow authenticated all access to solicitacoes_novo_insumo" 
ON public.solicitacoes_novo_insumo FOR ALL 
TO authenticated
USING (true)
WITH CHECK (true);
...
BEGIN
    ...
    LEFT JOIN (
        SELECT 
            ...
            COUNT(*) as pedidos,
            calcular_status_vip(cmd.cliente_id, v_data_inicio) as is_vip_calc
        FROM comandas cmd
